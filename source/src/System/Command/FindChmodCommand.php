...
class FindChmodCommand extends Command
{
    ...
    public function getCommand() : string
    {
        if (!$this->command) {
            ...
            $file = $this->getFile();
            $this->command = sprintf("/usr/bin/sudo /usr/bin/find -P %s -type d -exec chmod %s {} \\; && /usr/bin/sudo /usr/bin/find -P %s -type f -exec chmod %s {} \\;", escapeshellarg($file), escapeshellarg($directoryChmod), escapeshellarg($file), escapeshellarg($fileChmod));
        }
        return $this->command;
    }
