...
class AdminRemoteBackupAmazonS3EditType extends AbstractType
{
    public function buildForm(FormBuilderInterface $builder, array $options) : void
    {
        ...
        $builder->add("secretAccessKey", TextType::class, ["required" => false, "attr" => ["class" => "form-control form-control-lg"], "label" => "AWS Secret Access Key"]);
        $builder->add("bucket", TextType::class, ["required" => true, "attr" => ["class" => "form-control form-control-lg"], "label" => "Bucket", "constraints" => [new AppAssert\AmazonS3Bucket()]]);
        $regions = AwsRegions::getRegions();
        $regionChoices = [];
        foreach ($regions as $region => $name) {
            $regionChoices[$name] = $region;
        }
        $builder->add("region", ChoiceType::class, ["required" => true, "attr" => ["class" => "form-select form-select-lg"], "label" => "Region", "choices" => $regionChoices]);
