    ...
    public function validate($value, Constraint $constraint)
    {
        try {
            ...
            if (false === empty($accessKey) && false === empty($secretAccessKey)) {
                ...
                $s3Client = new S3Client(['version' => 'latest', 'region' => $region, 'credentials' => $credentials]);
                $bucketName = $form->get('bucket')->getData();
                $bucketFound = $s3Client->doesBucketExist($bucketName);
