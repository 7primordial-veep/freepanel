...
EOD;
    ...
    /**
     * @ORM\Column(name="wp_auto_update", type="string", length=16, nullable=true)
     */
    private $wpAutoUpdate = 'none';
    /**
    ...
    private $memoryQuota;
    /**
     * @ORM\Column(name="disk_quota_mb", type="integer", nullable=true)
     */
    private $diskQuotaMb;
    /**
     * @ORM\Column(name="disk_quota_project_id", type="integer", nullable=true)
     */
    private $diskQuotaProjectId;
    /**
     * @ORM\Column(name="last_disk_usage_mb", type="integer", nullable=true)
     */
    private $lastDiskUsageMb;
    /**
     * @ORM\Column(name="last_disk_check_at", type="datetime", nullable=true)
     */
    private $lastDiskCheckAt;
    ...
    public function getWpAutoUpdate() : ?string
    {
        return $this->wpAutoUpdate ?: 'none';
    }
    public function setWpAutoUpdate(?string $mode) : void
    {
        $this->wpAutoUpdate = in_array($mode, ['none','security','all'], true) ? $mode : 'none';
    }
    public function getBlockedCountries() : ?string
    {
        return $this->blockedCountries;
    }
    public function setBlockedCountries(?string $blockedCountries) : void
    {
        if (true === is_string($blockedCountries)) {
            $codes = [];
            foreach (explode(',', strtoupper($blockedCountries)) as $code) {
                $code = preg_replace('/[^A-Z]/', '', trim($code));
                if (2 === strlen($code)) { $codes[$code] = $code; }
            }
            $blockedCountries = count($codes) ? implode(',', array_values($codes)) : null;
        }
        $this->blockedCountries = $blockedCountries;
    }
    public function getBlockedCountriesArray() : array
    {
        return true === empty($this->blockedCountries) ? [] : explode(',', $this->blockedCountries);
    }
    public function getCpuQuota() : ?int { return $this->cpuQuota; }
    public function setCpuQuota(?int $v) : void { $this->cpuQuota = $v; }
    public function getMemoryQuota() : ?int { return $this->memoryQuota; }
    public function setMemoryQuota(?int $v) : void { $this->memoryQuota = $v; }
    public function getDiskQuotaMb() : ?int { return $this->diskQuotaMb; }
    public function setDiskQuotaMb(?int $v) : void { $this->diskQuotaMb = $v; }
    public function getDiskQuotaProjectId() : ?int { return $this->diskQuotaProjectId; }
    public function setDiskQuotaProjectId(?int $v) : void { $this->diskQuotaProjectId = $v; }
    public function getLastDiskUsageMb() : ?int { return $this->lastDiskUsageMb; }
    public function setLastDiskUsageMb(?int $v) : void { $this->lastDiskUsageMb = $v; }
    public function getLastDiskCheckAt() : ?\DateTime { return $this->lastDiskCheckAt; }
    public function setLastDiskCheckAt(?\DateTime $v) : void { $this->lastDiskCheckAt = $v; }
    ...
    {
        if (empty($this->dockerEnv)) {
            return [];
        }
        $decoded = json_decode($this->dockerEnv, true);
        return is_array($decoded) ? $decoded : [];
    }
    public function setDockerEnv(?array $env) : void
    {
        $this->dockerEnv = empty($env) ? null : json_encode($env);
    }
    public function getDockerVolumes() : array
    {
        if (empty($this->dockerVolumes)) {
            return [];
        }
        $decoded = json_decode($this->dockerVolumes, true);
        return is_array($decoded) ? $decoded : [];
    }
    public function setDockerVolumes(?array $volumes) : void
    {
        $this->dockerVolumes = empty($volumes) ? null : json_encode($volumes);
    }
    ...
    public function getSecurityHeaders() : ?string
    {
        return $this->securityHeaders;
    }
    public function setSecurityHeaders(?string $securityHeaders) : void
    {
        $this->securityHeaders = $securityHeaders;
    }
    public function getSecurityHeadersConfig() : array
    {
        $json = $this->securityHeaders;
        if (empty($json)) {
            return [];
        }
